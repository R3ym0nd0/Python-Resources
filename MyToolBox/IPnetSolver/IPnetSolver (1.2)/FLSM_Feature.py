...
def FLSM() -> None:
    def calculate_prefix_length(ip: str) -> int:
        ...
        while True:
            try:
                ...
                num_subnet: int = input(f"--> Please enter the number of subnets (1 to {ip.num_addresses}). Type 'q' to quit: ").strip().lower()
                # Options
    ...
    def ask_to_convert_to_binary(ip: str, prefix: int) -> None:     
        while True:
            ...
            user: str = input("--> Do you want to convert the Network Address, Broadcast Address, and Subnet Mask into binary? (Y/N only) ").strip()
            if user.lower() == "y":
