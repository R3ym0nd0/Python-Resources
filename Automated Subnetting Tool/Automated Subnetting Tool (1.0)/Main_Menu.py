...
def Subnetting():           
    ...
    def main_menu():
        while True:
            ...
            print("[5] Leave\n")
            user = input("--> Pick what you want to use (1 - 5 Only): ").strip()
            if user == "1":
                separator()
                print("--> Fixed-Length Subnet Masking(FLSM) <--\n")
                from FLSM_Code import FLSM # FLSM Module
                FLSM()
            elif user == "2":
                separator()
                print("--> Variable-Length Subnet Masking(VLSM) <--\n")
                from VLSM_Code import VLSM # VLSM Module
                VLSM()
            elif user == "3":
                separator()
                print("--> Supernetting <--\n")
                from Supernetting_Code import Supernetting # Supernetting Module
                Supernetting()
            elif user == "4":
                separator()
                print("--> IP Address/Numbers to Binary Translator <--\n")
                from Binary_Translator import Binary_Translator # Binary_Translator Module
                Binary_Translator()
            elif user == "5":
                break
